...
| The card is one component in both layouts | `.is-list` on the grid turns the same `cardHtml` on its side. A dedicated list-row component would have been the second thing to keep in step with every future change to a card, and the layout is genuinely only a rotation: name and minutes left, tags beside them, the same foot of buttons moved to the right and divided by the same hairline |
