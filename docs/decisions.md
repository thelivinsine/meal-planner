...
| No focus restoration in the storage listener | A redraw destroys focus and six places here put it back, so leaving it out looks like the defect this project keeps shipping. It is not: the event only ever arrives in a tab the user is **not** in — they are in the tab that did the saving — so there is no live focus to restore. The tools row is never redrawn either way, so a half-typed search survives regardless. Asserted rather than assumed |
