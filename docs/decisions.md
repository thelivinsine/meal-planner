...
| The slot picker opens filtered to its meal, visibly | The complaint was exact: tap *Add a breakfast* and all fifty recipes appear. It could have been a hidden rule in the render, and instead it is a ticked box in the *Meal* dropdown with the filter row open — so the short list explains itself, and the one person who wants a breakfast at dinner can untick it. It is rewritten on every open, because last time's search is not this time's question |
...
there. The consequence here was that the shared `:focus-visible` ring, a 3px accent band, appeared
