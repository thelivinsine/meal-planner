...
| The tag pills went to `--surface-sunk`, not up with the controls | A tag is a **label, not a control**, and it has no border — so it cannot follow the controls up towards white, where a borderless pill measures 1.07 on a white card and simply is not there. The recessed shade is the one place a borderless pill still reads in a light theme, 1.20 down. That leaves the tags as the only fill inside a card that goes *down*, which is the right shape: everything you can press is raised, everything that is only a word is sunk |
| The page went a step deeper, to `#f4f0e9` | Once every control on a card sat above the page, the page was the only mid-tone left on screen and a white card lifted off it by just **1.11**. 1.14 now — still inside the 1.07–1.25 band both light references sit in, and it buys the room `--control` and `--surface-past` needed above it. It costs two ratios: `--accent-soft` on the page falls to 1.04 and `--surface-past` rises to 1.07, both already hairline-carried pairs. The value is written **twice** — the token, and the `theme-color` meta in `index.html` — and `check.mjs` caught the second copy, which is the whole reason that check exists |
