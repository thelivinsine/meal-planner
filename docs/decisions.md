...
| Decision | Why |
|---|---|
| The page is warm, the cards are near-white | Tried the other way twice. A tinted page behind near-white cards first failed at a 1.12 edge, so v1 of this palette put a heavy peach on every tile — which read as one flat cream wash the moment the cards got large, as the screenshots showed. Back to a warm page and light cards, this time measured: 1.16 page to card, 1.11–1.26 card to tile |
...
| One box per level | A card gets the border; what sits inside is a filled tile or a hairline and a label, never a second outline. Nested boxes were the main thing wrong with v1's week, and it's why no day card wraps the meals — the `<h2>` above them already names the day |
...
4. The save star — one lookup covers the week, both recipe lists and the open dialog, so the fix
...
were one: the save star, and then Today — which was missed precisely because it used to be static
