...
| The add button is its glyph alone under 620px | Full width it was 340px of accent across a 360px screen — louder than the recipe name, which is the thing being chosen, and in a list it took two thirds of the row and pushed every name onto two lines. `ICON.calendarPlus` alone is 44px, and the foot goes from 340px to 120px. Three things made it safe rather than clever: the glyph and the label are **both in the markup at every width** so a window dragged across 620px needs no redraw; the label is *hidden, not dropped*, with `aria-label` carrying the same sentence at both widths, so the narrow button is never a nameless plus sign; and the rule sets its own width to `var(--tap)`, because it sits after the `pointer: coarse` block and would otherwise undercut the 44px floor that block exists to hold |
...
| `--control` exists because a track and a control point opposite ways in dark | `--surface-sunk` was doing two jobs: the recessed *track* of the At-a-glance bar, and the rest fill of every small control on a card — the search field, the tags, the chips, the filter summaries. In light both go **down** from the card, so one token was right. In dark they part company: a track still recedes, but a control is supposed to be **raised**, and `--surface-sunk` left every chip, tag and search field 1.14 *below* its card and only **1.08** above the page — the "hole punched through the tile" this project already documented for `--bg`, one token along. So the control half became `--control`: `#f0eae1` in light, which is the value it already had, so **nothing in the light theme moved**; `#383838` in dark, which is PowerToys' own "input inside a card" fill, 1.21 above the card. This is finding (a) and (b) of [dark-mode-reference.md §8](dark-mode-reference.md#8-against-mises-current-dark-tokens), and the shape of the answer was in the report: the ladder needed a step above `--surface` and there wasn't one |
...
| Hover stopped being an accent wash | `.btn:hover` and `.icon-btn:hover` both filled with `--accent-soft`, and the layout toggle's **pressed** state is `--accent-soft` too — so hover and pressed were the same two colours, and you could not tell which layout was on by looking at the one under the pointer. Both hovers are `--control` now, neutral, with the accent border still carrying the colour half on `.btn`. It also fixes a number: in dark, `--accent-soft` is 1.12 on a card and that hover barely registered, where `--control` is 1.21. **Every selected state in this app is an accent fill**, so a hover that washes accent is borrowing the one signal that means "this is the chosen one" |
